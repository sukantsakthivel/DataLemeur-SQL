...
time_spent
FROM activities a
JOIN age_breakdown e using (user_id) 
WHERE a.activity_type <> 'chat'),
CTE_2 AS
(
  SELECT USER_ID, SUM(TIME_SPENT) AS SEND_SUM FROM CTE
  WHERE ACTIVITY_TYPE = 'send'
  GROUP BY user_id
),
CTE_3 AS
(
  SELECT USER_ID, SUM(TIME_SPENT) AS OPEN_SUM FROM CTE
...
(
  SELECT USER_ID,
  ROUND(send_sum/(send_sum+open_sum)*100,2) as send_perc,
  ROUND(open_sum/(send_sum+open_sum)*100,2) as open_perc
  FROM CTE_2 
  JOIN CTE_3
  USING (USER_ID)
)
SELECT distinct a.age_bucket,
e.send_perc,
e.open_perc
from cte_4 e
join cte a
