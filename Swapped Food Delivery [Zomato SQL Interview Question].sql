...
WITH CTE AS (SELECT * 
FROM orders 
WHERE ORDER_ID%2!=0 AND ORDER_ID != (SELECT MAX(ORDER_ID) FROM ORDERS))
,
CTE_2 AS (SELECT *
FROM ORDERS
WHERE ORDER_ID%2=0),

CTE_3 AS (SELECT ORDER_ID,ITEM FROM ORDERS
ORDER BY ORDER_ID DESC
LIMIT 1)

SELECT ORDER_ID-1 AS ORDER_ID,ITEM FROM CTE_2 
UNION
SELECT CTE.ORDER_ID+1 AS ORDER_ID,ITEM FROM CTE
UNION
SELECT * FROM CTE_3
ORDER BY ORDER_ID;
